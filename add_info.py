...
URL_PROJ = 'https://www.dropbox.com/s/y4va4tzi67wi0ct/proj_for_google2.jpg?raw=1'
URL_PROJ2 = 'https://www.dropbox.com/s/5tdrts1z4l0ni4c/proj2_for_google2.jpg?raw=1'
URL_RECON = 'https://www.dropbox.com/s/2ph2lsu6h80y6la/recon_for_google2.jpg?raw=1'
...
def read_recon(pars):
    ...
    try:
        ...
        # read z slices
        # take size
        tmp = dxchange.read_tiff(f'{dirname}_recgpu/{basename}_rec/r_00000.tiff').copy()
        ...
        pars['y'] = np.zeros((h,w),dtype='float32')
        pars['x'] = np.zeros((h,w),dtype='float32')    
        for j in range(h):
            zz = dxchange.read_tiff(f'{dirname}_recgpu/{basename}_rec/r_{j:05}.tiff')
            pars['y'][j,:] = zz[pars['idy']]
            pars['x'][j,:] = zz[:,pars['idx']]        
    except:
        pars['z'] = None
        pars['y'] = None
        pars['x'] = None
...
def plot_recon(pars):
    ...
    grid = fig.add_gridspec(3, 1, height_ratios = [1, 1, pars['width']/pars['height']])
    slices = ['x','y','z']
    # autoadjust colorbar values according to a histogram
    mmin,mmax = utils.find_min_max(np.concatenate((pars[slices[0]],pars[slices[2]],pars[slices[2]])))  
    ...
    for k in range(3):
        ...
        if(k<2):
            ax.set_xticklabels([])                
        if k==2:# z slices
            ax.plot([w*8.7/10,w*8.7/10+scalebar*1000/pars['resolution']/pars['binning']],[w*9.5/10,w*9.5/10],'r')
            txt = ax.text(w*8.7/10,w*9.15/10,f'{scalebar}um',color='red',fontsize=14)
            txt.set_path_effects([PathEffects.withStroke(linewidth=1, foreground='w')])
        else: #x,y slices
            ax.plot([w*8.7/10,w*8.7/10+scalebar*1000/pars['resolution']/pars['binning']],[h*9.5/10,h*9.5/10],'r')
            txt = ax.text(w*8.7/10,h*9.1/10,f'{scalebar}um',color='red',fontsize=14)
            txt.set_path_effects([PathEffects.withStroke(linewidth=1, foreground='w')])    
    plt.savefig(FILENAME_RECON,bbox_inches='tight',pad_inches = 0,dpi=150) 

def upload_proj_micro(dbx):
    with open(FILENAME_PROJ2, 'rb') as f:
            dbx.files_upload(f.read(), '/'+FILENAME_PROJ2,dropbox.files.WriteMode.overwrite)

def upload_proj_nano(dbx):
    with open(FILENAME_PROJ, 'rb') as f:
            dbx.files_upload(f.read(), '/'+FILENAME_PROJ,dropbox.files.WriteMode.overwrite)
    
def upload_recon(dbx):
    with open(FILENAME_RECON, 'rb') as f:
            dbx.files_upload(f.read(), '/'+FILENAME_RECON,dropbox.files.WriteMode.overwrite)
...
def main():
    ...
    if len(sys.argv)==5:
        pars['idx'] = int(sys.argv[2])
        pars['idy'] = int(sys.argv[3])
        pars['idz'] = int(sys.argv[4])
    else:
        pars['idx'] = -1
        pars['idy'] = -1
        pars['idz'] = -1
    ...
    upload_proj_nano(dbx)
    url_list.append(URL_PROJ)                
    ...
    if pars['proj2'] is not None:    
        ...
        upload_proj_micro(dbx)  
        url_list.append(URL_PROJ2) 
        magnitude_list.append(210)        
        posx_list.append(0)            
